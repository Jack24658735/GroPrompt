...
class OnlineRefer(nn.Module):
    ...
    def forward_single_frame(self, samples: NestedTensor, track_res, captions, targets):
        ...
        # Transformer
        query_embeds = track_res['pos_embedding']
        ...
        hs, memory, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, inter_samples = \
            self.transformer(srcs, text_embed, masks, poses, query_embeds, track_res['ref_pts'])
    ...
    def post_process_single_image(self, frame_res, track_res, is_last):
    ...
    def inference(self, samples:NestedTensor, track_res, captions, targets):
        ...
        frame_res = self.forward_single_frame(frame, track_res, captions, targets)
        ...
        for frame_index in range(num_frames):
            ...
            if self.use_checkpoint_for_more_frames and frame_index < num_frames - 1:
                def fn(frame, *args):
                    ...
                    frame_res = self.forward_single_frame(frame, tmp, captions, target_i)
                    return (
                ...
                frame_res = {
                    ...
                    'aux_outputs': [{
                        'pred_logits': tmp[5 + i],
                        'pred_boxes': tmp[5 + 3 + i],
                        'pred_masks': tmp[5 + 3 + 3 + i]
                    } for i in range(3)],
                }
            else:
                frame = nested_tensor_from_videos_list(frame)
                frame_res = self.forward_single_frame(frame, track_res, captions, target_i)
            track_res = self.post_process_single_image(frame_res, track_res, is_last)
