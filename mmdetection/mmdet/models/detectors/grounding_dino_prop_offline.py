...
class GroundingDINOPropOffline(DINO):
    ...
    def pre_decoder(
        ...
        if self.training:
            ### TODO: offline propagation

            # 1. first loop through all frames (e.g., 5) to obtain 5 * num_prop queries
            # 2. loop again and combine them (e.g., original + num_prop) for the input features
            topk_indices_list_prop = []
            topk_score_list_prop = []
            topk_coords_unact_list_prop = []
            topk_coords_list_prop = []
            query_list_prop = []
            for i in range(bs):
                # list len = num_frames, where each element is torch.Size([1, num_prop, 4]) or torch.Size([1, num_prop, 256])
                ...
                # select from here => 900 to num_prop
                query_for_prop = query[0, topk_indices_prop[0]].detach().unsqueeze(0)
                topk_indices_list_prop.append(topk_indices_prop)
                topk_score_list_prop.append(topk_score_prop)
                topk_coords_unact_list_prop.append(topk_coords_unact_prop)
                topk_coords_list_prop.append(topk_coords_prop)
                query_list_prop.append(query_for_prop)
            ...
            for i in range(bs):
                ...
                # NOTE: select 900 -  (bs - 1) * num_prop features to be used here
                topk_indices_curr_frame = torch.topk(enc_outputs_coord_unact_curr_frame.max(-1)[0], k=self.num_queries - (bs - 1) * self.num_prop, dim=1)[1]
                ...
                # TODO: exclude the current frame features
                topk_indices_prop = topk_indices_list_prop[0:i] + topk_indices_list_prop[i+1:]
                topk_score_prop = topk_score_list_prop[0:i] + topk_score_list_prop[i+1:]
                topk_coords_unact_prop = topk_coords_unact_list_prop[0:i] + topk_coords_unact_list_prop[i+1:]
                topk_coords_prop = topk_coords_list_prop[0:i] + topk_coords_list_prop[i+1:]
                query_prop = query_list_prop[0:i] + query_list_prop[i+1:]
                # from list of tensor to become a tensor
                topk_indices_prop = torch.cat(topk_indices_prop, dim=1)
                topk_score_prop = torch.cat(topk_score_prop, dim=1)
                topk_coords_unact_prop = torch.cat(topk_coords_unact_prop, dim=1)
                topk_coords_prop = torch.cat(topk_coords_prop, dim=1)
                query_prop = torch.cat(query_prop, dim=1)

                
                # TODO: concat with prop. features (select not curr "i" frame features)
                # concat with curr frame feature and overwrite
                topk_indices = torch.cat((topk_indices_prop, topk_indices_curr_frame), dim=1)
                topk_score = torch.cat((topk_score_prop, topk_score_curr_frame), dim=1)
                topk_coords_unact = torch.cat((topk_coords_unact_prop, topk_coords_unact_curr_frame), dim=1)
                topk_coords = torch.cat((topk_coords_prop, topk_coords_curr_frame), dim=1)
                query = torch.cat((query_prop, query_curr_frame), dim=1)
