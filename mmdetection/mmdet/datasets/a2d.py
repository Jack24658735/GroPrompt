...
class A2DSentencesDataset(Dataset):
    ...
    """
    def __init__(self, a2d_dict, *args, **kwargs):
        super(A2DSentencesDataset, self).__init__()
        self.img_folder = a2d_dict['img_folder']
        dataset_path = str(self.img_folder)
        self.mask_annotations_dir = os.path.join(dataset_path, 'a2d_annotation_with_instances')
        self.videos_dir = os.path.join(dataset_path, 'Release/clips320H')
        self.ann_file = a2d_dict['ann_file']
        self.text_annotations = self.get_text_annotations()
        self.num_frames = a2d_dict['num_frames']
        self.max_skip = a2d_dict['max_skip']
        self.subset = a2d_dict['subset']
        print(f'\n {self.subset} sample num: ', len(self.text_annotations))  
        print('\n') 
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            # DONE: build up neg texts
            filtered_annotations = [annotation for annotation in self.text_annotations if annotation[1] == video_id and annotation[3] != instance_id]
            all_exp_for_obj = [anno[0] for anno in filtered_annotations]
            ...
            if self.subset == 'train':
                ...
                sample_id_after = random.randint(1, 3)
                local_indx = [min(vid_len - 1, frame_id + sample_id_after)]
                sample_indx.extend(local_indx)
            ...
            for j in range(self.num_frames):
                ...
                ### BUG: THE differneces from OnlineRefer,
